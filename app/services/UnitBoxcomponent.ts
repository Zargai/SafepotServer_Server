...
export default class UnitBoxcomponentService {
  constructor(
    @Inject('unitboxcomponentModel') private UnitsboxComponent: Models.UnitsboxComponentModel,
  ) { }
  //for adding data
  public async Add(InputDTO: IUnitInput): Promise<{ Record: IUnit }> {
    const count = await this.UnitsboxComponent.countDocuments();
    var id =1;
    if(count>0){
        const data1 = await this.UnitsboxComponent.find().limit(1).sort({$natural:-1})
      ...
      console.log("dataaa",data)
       const Record = await this.UnitsboxComponent.create({...data});
  ...
  public async update(InputDTO: IUnitInput): Promise<{ message: string, success: boolean }> {
    try {
     ...
     const  id =InputDTO.id
      const Record = await this.UnitsboxComponent.updateOne({ "id": id }, { ...InputDTO })
 ...
 //for deletion
 public async delete(InputDTO: IUnitInput): Promise<{  success: boolean; }> {
    try {    
        const id =InputDTO.id
      const userRecord = (await this.UnitsboxComponent.deleteOne({ "id": id }))
  ...
  //getalldata
  public async getall(): Promise<{ Record: Array<IUnitInput>; }> {
    try {
      const Record = await this.UnitsboxComponent.find()
      if (!Record) {
