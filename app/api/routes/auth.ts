...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const { userRecord } = await authServiceInstance.SignUp(
  ...
  route.delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { success } = await authServiceInstance.deleteuser(
          req.params.systemId as string
        );
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const { message, success } = await authServiceInstance.updateuser(
          systemId as string,
          req.body as IUserInput
        );
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { systemId } = req.body;
        const { userRecord } = await authServiceInstance.getuser(
          systemId as string
        );
        return res.json(userRecord).status(200);
      } catch (e) {
  ...
  var storage = multer.diskStorage({
    ...
    destination: function (req, file, cb) {
      cb(null, "../safepots/public/uploads");
    },
